...
/*
 * $(function() { $( "#datepicker" ).datepicker(); $( "#datepicker-1"
 * ).datepicker(); $( "#datepicker-2" ).datepicker(); $( "#datepicker-3"
 * ).datepicker(); });
 */
(function($) {
...
$('#tab0').load(UI.tab0);
/*
 * $('#tab1').load(UI.tab1); $('#tab2').load(UI.tab2); $('#tab3').load(UI.tab3);
 * $('#tab4').load(UI.tab4); $('#tab5').load(UI.tab5); $('#tab6').load(UI.tab6);
 * $('#tab7').load(UI.tab7); $('#tab8').load(UI.tab8); $('#tab9').load(UI.tab9);
 */
...
function setModalsAndBackdropsOrder() {
    ...
    $('.modal.in').each(
		...
		$modal.next('.modal-backdrop.in').addClass('hidden').css(
			'zIndex', modalZIndex - 1);
	    });
    $('.modal.in:visible:last').focus().next('.modal-backdrop.in').removeClass(
	    'hidden');
}

function doinitFrom() {
    $uniformed = $(".formstyle").find(
	    "input.unistyle, textarea, select, a.uniformTest");
    $uniformed.uniform();
}
...
addLoading = function(element, typeLoading, additionClass) {
    ...
    jElement
	    .append('<div class="overlay-bg'
		    + additionClass
		    + '"><div class="bg"></div><img src="/images/loading-line.gif" class="loading-small overlay-small-loading" /></div>');
